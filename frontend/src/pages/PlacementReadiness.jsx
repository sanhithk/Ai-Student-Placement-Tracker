    ...
    <div className={`flex flex-col items-center ${isLg ? 'gap-4' : 'gap-2'}`}>
      <div className={`relative ${isLg ? 'w-36 h-36' : 'w-24 h-24'} flex items-center justify-center`}>
        ...
        <div className="absolute flex flex-col items-center justify-center">
          <span className={`${isLg ? 'text-4xl' : 'text-xl'} font-black text-slate-900 dark:text-white`}>{score}</span>
        </div>
      </div>
      {title && (
...
const PlacementReadiness = () => {
  ...
  if (loading) return <div className="text-slate-600 dark:text-slate-400 text-center py-20">Loading your Readiness Profile...</div>;
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500 max-w-6xl mx-auto">
      <div>
        ...
        <p className="text-slate-600 dark:text-slate-400 mt-2">A holistic 360° view of your placement preparedness based on your platform activity.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        
        {/* Main Score Card */}
        <Card className="bg-gradient-to-b from-slate-800 to-slate-900 border-slate-200 dark:border-slate-700 lg:col-span-1 shadow-2xl flex flex-col justify-center items-center p-8 text-center">
          <ScoreRing score={data.overallScore} size="lg" colorClass={data.overallScore > 75 ? "text-emerald-500" : "text-amber-500"} />
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white mt-6 mb-2">Overall Readiness</h2>
        ...
        <div className="lg:col-span-2 grid grid-cols-2 md:grid-cols-3 gap-4">
          <Card className="flex items-center justify-center py-6 bg-slate-50/40 dark:bg-slate-800/40">
          ...
          <Card className="flex flex-col items-center justify-center py-6 bg-slate-50/40 dark:bg-slate-800/40 gap-4">
            <ScoreRing score={data.breakdown.aptitude} title="Aptitude" icon={Target} colorClass="text-teal-400" />
            <span className="text-[10px] text-slate-500 bg-white dark:bg-slate-900 px-2 py-1 rounded">Self-Reported</span>
          </Card>
        </div>
      </div>
              ...
              <div className="grid gap-4 md:grid-cols-3">
                {aiRecs.recommendations.map((rec, idx) => (
                  <motion.div 
                    ...
                    key={idx} 
                    className="bg-white/50 dark:bg-slate-900/50 border border-slate-200/50 dark:border-slate-700/50 p-5 rounded-xl relative overflow-hidden"
                  >
                    ...
                    <div className="flex justify-between items-start mb-3">
                      <span className="text-sm font-bold text-slate-800 dark:text-slate-200 bg-slate-50 dark:bg-slate-800 px-2 py-1 rounded">{rec.area}</span>
                    ...
                    <p className="text-slate-600 dark:text-slate-400 text-sm leading-relaxed">{rec.advice}</p>
                  </motion.div>
                ))}
              </div>
            </div>
