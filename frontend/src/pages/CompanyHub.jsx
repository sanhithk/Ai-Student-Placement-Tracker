...
const CompanyHub = () => {
  ...
  if (loading) return <div className="text-slate-400 text-center py-20">Loading Top Companies...</div>;
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500">
      <div className="text-center space-y-4 max-w-2xl mx-auto">
        ...
        <h1 className="text-4xl font-extrabold text-white">Company Prep Hub</h1>
        <p className="text-slate-400 text-lg">Detailed hiring workflows, recommended topics, and personalized readiness matching for top tech companies.</p>
      </div>
        ...
        {companies.map((company, idx) => (
          <Link to={`/company/${company.id}`} key={company.id}>
            <motion.div
              ...
              className="h-full"
            >
              <Card className="h-full bg-slate-900/50 hover:bg-slate-800/80 hover:border-primary-500/50 transition-all duration-300 cursor-pointer group">
                <CardBody className="p-6 flex flex-col items-center text-center h-full">
                  ...
                  <h2 className="text-xl font-bold text-white mb-1 group-hover:text-primary-400 transition-colors">{company.name}</h2>
                  <span className="text-xs font-medium text-slate-400 bg-slate-800 px-2 py-1 rounded-full mb-4">
                    {company.tier}
                  </span>
                  
                  <div className="mt-auto w-full border-t border-slate-800 pt-4 flex items-center justify-between">
                    <div className="flex flex-col items-start">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span className="text-sm font-bold text-slate-300">{company.difficultyScore}/100</span>
                      </div>
                    </div>
