    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <Card>
        <CardBody className="p-6">
          <form onSubmit={handleGenerate} className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Job Description <span className="text-red-400">*</span></label>
              <textarea 
                rows="6"
                placeholder="Paste the full job description here..." 
                className="w-full px-4 py-3 bg-white/50 dark:bg-slate-900/50 text-slate-900 dark:text-white border border-slate-200 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-primary-500 outline-none transition-shadow font-mono text-sm"
      ...
      {powData && (
        <div className="space-y-6 mt-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <h2 className="text-2xl font-bold text-slate-900 dark:text-white mb-4 border-b border-slate-200 dark:border-slate-700 pb-2">Your Micro-Project Strategy</h2>
          ...
          <Card className="border-primary-500/30 shadow-lg shadow-primary-900/10">
            <CardBody className="p-8">
              <div className="flex items-start gap-4 mb-6">
                ...
                <div>
                  <h3 className="text-2xl font-bold text-slate-900 dark:text-white">{powData.projectTitle}</h3>
                  <p className="text-slate-700 dark:text-slate-300 mt-2 leading-relaxed">{powData.description}</p>
                </div>
              </div>
              ...
              <div className="mb-8">
                ...
                <div className="flex flex-wrap gap-2">
                  {powData.techStack.map((tech, i) => (
                    <span key={i} className="px-3 py-1 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-slate-200 rounded-lg text-sm font-medium">
                      {tech}
                    </span>
                  ))}
                </div>
                ...
                <div className="space-y-4">
                  {powData.architectureSteps.map((step, i) => (
                    <div key={i} className="flex gap-4">
                      <div className="w-8 h-8 rounded-full bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-300 flex items-center justify-center font-bold text-sm shrink-0 border border-slate-200 dark:border-slate-700">
                        {i + 1}
                      </div>
              ...
              <div className="bg-white/60 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-700 rounded-xl p-6 relative group">
                <h4 className="text-sm font-semibold text-primary-400 uppercase tracking-wider mb-3">Recruiter Outreach Message</h4>
                <p className="text-slate-700 dark:text-slate-300 whitespace-pre-wrap font-serif italic text-sm leading-relaxed bg-white dark:bg-slate-900 p-4 rounded-lg border border-slate-200 dark:border-slate-800">
                  {powData.recruiterMessage}
                </p>
                <button 
                  onClick={copyToClipboard}
                  className="absolute top-6 right-6 p-2 bg-slate-50 dark:bg-slate-800 hover:bg-primary-900/50 text-slate-600 dark:text-slate-400 hover:text-primary-400 rounded-lg transition-colors flex items-center gap-2 text-xs font-medium border border-slate-200 dark:border-slate-700"
                >
