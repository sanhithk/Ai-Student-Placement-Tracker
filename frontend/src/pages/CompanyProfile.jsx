...
const CompanyProfile = () => {
  ...
  if (loading) return <div className="text-slate-600 dark:text-slate-400 text-center py-20">Loading Company Profile...</div>;
  if (!company) return <div className="text-red-400 text-center py-20">Company not found.</div>;

  return (
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-500">
      
      <Link to="/companies" className="inline-flex items-center text-slate-600 dark:text-slate-400 hover:text-primary-400 transition-colors text-sm font-medium">
      ...
      <div className="flex flex-col md:flex-row items-center md:items-start gap-8 bg-white/50 dark:bg-slate-900/50 p-8 rounded-3xl border border-slate-200 dark:border-slate-800">
        ...
        <div className="flex-1 text-center md:text-left space-y-2">
          ...
          <div className="flex flex-wrap justify-center md:justify-start gap-2 pt-2">
            ...
            <span className="bg-slate-50 dark:bg-slate-800 text-slate-700 dark:text-slate-300 border border-slate-200 dark:border-slate-700 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider">Difficulty: {company.difficultyScore}/100</span>
          </div>
        </div>
        
        {/* Match Score Widget */}
        <div className="bg-slate-50/80 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 p-5 rounded-2xl flex items-center gap-6 shadow-xl shrink-0">
          <div>
            <p className="text-sm text-slate-600 dark:text-slate-400 font-medium mb-1">Your Match Score</p>
          ...
          <div className="relative w-16 h-16 flex items-center justify-center">
            ...
            <span className="absolute text-lg font-black text-slate-900 dark:text-white">{matchData.matchPercentage}%</span>
          </div>
        </div>
      </div>
      ...
      <Card className="border-primary-900/30">
        <CardHeader className="bg-primary-900/10 border-b border-primary-900/20 flex justify-between items-center">
          ...
          {!roadmapData && (
            <button onClick={generateRoadmap} disabled={roadmapLoading} className="bg-primary-600 hover:bg-primary-500 text-slate-900 dark:text-white text-xs px-4 py-2 rounded-lg font-bold flex items-center shadow-lg shadow-primary-900/20 disabled:opacity-50 transition-colors">
        ...
        <CardBody className={roadmapData ? "p-6" : "p-12 text-center"}>
          {!roadmapData ? (
            ...
            <div className="space-y-8 animate-in slide-in-from-bottom-4 duration-500">
              ...
              <div>
                <h4 className="text-slate-800 dark:text-slate-200 font-bold mb-4">Your Step-by-Step Plan</h4>
                <div className="space-y-4">
                  {roadmapData.roadmap.map((step, idx) => (
                    <motion.div 
                      ...
                      key={idx} 
                      className="bg-slate-50/40 dark:bg-slate-800/40 border border-slate-200/50 dark:border-slate-700/50 p-4 rounded-xl flex gap-4"
                    >
                      ...
                      <div>
                        <h5 className="text-slate-900 dark:text-white font-bold mb-1">{step.title}</h5>
          ...
          <CardBody className="p-6">
            ...
            <div className="space-y-3">
              {matchData.insights.map((insight, idx) => (
                <div key={idx} className="flex items-start gap-3 bg-white/50 dark:bg-slate-900/50 p-3 rounded-lg border border-slate-200/50 dark:border-slate-800/50">
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card>
            ...
            <CardBody className="p-0">
              <div className="relative pl-8 pr-6 py-6 border-l-2 border-slate-200 dark:border-slate-800 ml-6 space-y-8">
                {company.hiringWorkflow.map((step, idx) => (
                  <div key={idx} className="relative">
                    <div className="absolute -left-[41px] top-0 w-8 h-8 bg-white dark:bg-slate-900 border-2 border-sky-500 rounded-full flex items-center justify-center shadow-[0_0_15px_rgba(14,165,233,0.3)]">
                      <span className="text-sky-400 font-bold text-xs">{step.step}</span>
                    </div>
                    <div>
                      <div className="flex flex-wrap items-baseline gap-3 mb-2">
                        <h3 className="text-lg font-bold text-slate-800 dark:text-slate-200">{step.name}</h3>
                        <span className="text-xs px-2 py-0.5 rounded-full bg-slate-50 dark:bg-slate-800 text-slate-600 dark:text-slate-400 font-medium border border-slate-200 dark:border-slate-700">{step.duration}</span>
          ...
          <Card>
            ...
            <CardBody className="space-y-4">
              {company.hrQuestions.map((q, idx) => (
                <div key={idx} className="bg-slate-50/40 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-700 p-4 rounded-xl">
                  <p className="text-slate-800 dark:text-slate-200 italic font-medium">"{q}"</p>
                </div>
              ))}
            </CardBody>
          </Card>
            ...
            <CardBody>
              <ul className="space-y-3">
                {company.techSubjects.map((subject, idx) => (
                  <li key={idx} className="flex items-center gap-3 text-slate-700 dark:text-slate-300 text-sm bg-slate-50/30 dark:bg-slate-800/30 p-2.5 rounded-lg border border-slate-200 dark:border-slate-800">
